...
import { getRegulars } from 'apiSdk/regulars';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['end-customer', 'car-owner'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<RegularInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/regulars` : null),
    () => getRegulars({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Regulars',
      path: isTenantUser ? `/regulars/view/${data?.[0]?.id}` : '/regulars',
      entity: 'regular',
