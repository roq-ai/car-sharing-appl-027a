...
import { getRegulars, deleteRegularById } from 'apiSdk/regulars';
import { RegularInterface } from 'interfaces/regular';

type ColumnType = ColumnDef<RegularInterface, unknown>;

interface RegularListPageProps {
...
export function RegularListPage(props: RegularListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RegularInterface>>(
    () => `/regulars?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: RegularInterface) => {
    if (hasAccess('regular', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/regulars/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/regulars/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('regular', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/regulars/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('regular', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
