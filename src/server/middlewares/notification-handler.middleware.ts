...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'Booking.create': { roles: ['end-customer'], key: 'booking_id', tenantPath: [], userPath: [] },
  'Booking.update': { roles: ['car-owner'], key: 'booking_id', tenantPath: [], userPath: [] },
  'Booking.delete': { roles: ['end-customer'], key: 'booking_id', tenantPath: [], userPath: [] },
};

const ownerRoles: string[] = ['end-customer'];
const customerRoles: string[] = ['guest'];
const tenantRoles: string[] = ['car-owner', 'end-customer'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.regular.findFirst({
